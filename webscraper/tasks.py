...
def aldi_scrape(search_term):
    ...
    with webdriver.Chrome(service=service, options = option) as driver:
        ...
        try:
            ...
            item_details = {"name": "N/A", "image_name" : "N/A" ,"price": 0.00,
                        "unit_price": None, "url": url, "store": "aldi"}
...
def morrisons_scrape(search_term):
    ...
    with webdriver.Chrome(service=service, options = option) as driver:
        ...
        try:
            ...
            item_details = {"name": "N/A", "image_name": "N/A", "price": 0.00,
                            "unit_price": None, "url": url, "store": "morrisons"}
            results = [item_details]
            return results
...
def tesco_scrape(search_term):
    ...
    with webdriver.Chrome(service=service, options=option) as driver:
        ...
        if (soup.select("div.results-page.no-results")):
            ...
            item_details = {"name": "N/A", "image_name": "N/A" , "price": 0.00,
                        "unit_price": None, "url": url, "store": "tesco"}
            results = [item_details]
            return results
        ...
        for elem in items:
            ...
            try:
                unit_price = elem.select_one(
                    'div.price-per-quantity-weight').text
            except (NoSuchElementException,AttributeError):
                unit_price = "n/a"
            ...
            item_details = {"name": name, "image_name": img_name, "price": price,
                                    "unit_price": unit_price, "url": url, "store": "tesco"}
            results.append(item_details)
